    ...
    stage('Git Clone') {
      steps {
        checkout([
            ...
            url: 'https://github.com/NWQMC/schema-epa.git']]])
      }
    }
    stage('Download liquibase jar') {
      steps {
        sh '''mkdir $WORKSPACE/epa
          ...
          /usr/bin/tar xzf $WORKSPACE/epa/liquibase.tar.gz --overwrite -C $WORKSPACE/wqp_core
    ...
    stage('Run liquibase') {
      steps {
        script {
          ...
          env.EPA_DB_OWNER_USERNAME = secretsJson.DB_OWNER_USERNAME
          env.EPA_DB_OWNER_PASSWORD = secretsJson.DB_OWNER_PASSWORD
          ...
          sh '''
            ...
            chmod +x $WORKSPACE/liquibase/scripts/z2_epa_liquibase.sh
            $WORKSPACE/liquibase/scripts/z1_postgres_liquibase.sh
            $WORKSPACE/liquibase/scripts/z2_epa_liquibase.sh
            '''
        }
      }
    }
